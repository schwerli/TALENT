...
df = pd.read_excel('acc_bin.xlsx')
df = df.rename(columns=names_dict)
df_1 = pd.DataFrame(columns=['classifier_name','dataset_name','accuracy'])
cols = df.columns[1:]
for col in cols[1:]:
    for dataset in df['Dataset']:
        new_line = {
            'classifier_name':col,'dataset_name':dataset,'accuracy':df[df['Dataset']==dataset][col].values[0]
        }
        next_index = len(df_1)
        df_1.loc[next_index] = new_line

draw_cd_diagram(df_perf=df_1, title='Accuracy', labels=True,path = f'pics/cd_bin.pdf')